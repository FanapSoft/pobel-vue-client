...
const MockService = {
  init(context, inject) {
    ...
    mock.onGet(/\/api\/services\/app\/Targets\/GetAll\/?/).reply(data => {
      const token = data.headers.Authorization.replace("Bearer ", "");
      if (token !== "undefined") {
        let items = [];
    ...
    mock.onGet(/\/api\/services\/app\/TargetDefinitions\/Get\/?/).reply(data => {
      const token = data.headers.Authorization.replace("Bearer ", "");
      if (token !== "undefined") {
        return [200, {
          result: {
    ...
    mock.onGet(/\/api\/services\/app\/Questions\/GetRandomLabel\/?/).reply(data => {
      const token = data.headers.Authorization.replace("Bearer ", "");
      if (token !== "undefined") {
        let items = [];
        for(let i of Array( 10).keys())
    ...
    mock.onGet(/\/api\/services\/app\/Questions\/GetQuestions\/?/).reply(data => {
      const token = data.headers.Authorization.replace("Bearer ", "");
      if (token !== "undefined") {
        let items = [];
