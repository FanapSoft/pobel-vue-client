    ...
    <v-col cols="12" class="">
      ...
      <ul
        ...
        <li
          ...
          <span class="dataset-answers-history-items-result">{{ `${(item.Answer === 0) ? '⤫' : '✓'}` }}</span>
          <span class="dataset-answers-history-items-name">{{ (item.DeterminedLabel ? item.DeterminedLabel.Name.replace(/[0-9]/g, '').replace(/_/g, ' '): 'بی نام') }}</span>
